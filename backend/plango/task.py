...
RECORDED_VS_CURRENT = (
    ...
    "再写清是资料缺该值还是记录互相冲突。资料缺该值时，句中还要复述页上与所问属性相关的"
    "已核对事实（页上写明了什么），让用户看见查过什么、缺的只是所问属性，"
    "不要只以「未知」或「资料缺该值」收束。"
)
...
已有资料能支持的结论直接给出。用户问的数量、比较、时段、是否够用，先calculate再answer，不要只复述摘录。合计与加总的主数为页文全部费用项之和（各条件费一并计入），分场景口径作附注说明；不按自设场景排除条目，也不把口径差异标成未知。{RECORDED_VS_CURRENT}未知只限制依赖它的那一条结论；点明冲突与未知可以是完整答案。缺页值须answer，不要ask或问出发地。
...
calculate：operands为字符串，add/sum求和、multiply乘积、subtract/divide取前两项依次运算。time_add输入ISO日期时间或HH:MM加秒数，time_difference输入起止时间返回秒（纯时刻不猜跨天），date_weekday输入YYYY-MM-DD返回周一1到周日7。工具只验算，事实、单位、适用条件和比较含义由你依原文解释；两份记录的数字差不能用来选定其中一份作为当前适用值。求和与合计把页文逐项列出的费用全部计入，主数为全部费用项之和（含标注另收或条件性的附加费）；未说明场景时不要按堂食、外带等自设口径排除条目或挑选主数，场景口径作附注说明，不把口径差异标成未知。
