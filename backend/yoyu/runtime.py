...
from planora.persistence.database import agent_run
from planora.runtime import TERMINAL_PHASES, PlanoraRuntime
...
class DesktopRuntime(PlanoraRuntime):
    ...
    async def start(self):
        ...
        async with self.database.engine.begin() as connection:
            ...
            if self.settings.is_sqlite:
                columns = (
                    await connection.execute(text("PRAGMA table_info(yoyu_browser_binding)"))
                ).all()
                if "location_context" not in {row[1] for row in columns}:
                    await connection.execute(
                        text("ALTER TABLE yoyu_browser_binding ADD COLUMN location_context JSON")
                    )
        await super().start()
    ...
    async def _run_graph(self, run_id, resume=None, command_id=None):
        async with self._delivery_lock:
            ...
            self.model.system_prefix = (
                "YOYU 真实运行：Skill 仅提供任务提示，不能授予工具权限。禁止模拟价格、订单、预约号与履约成功；登录、验证码或不支持能力须暂停/人工接管。可用 Skill 目录："
