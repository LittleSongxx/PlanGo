...
def test_app_only_menu_preview_preserves_partial_fields_without_a_vision_loop(tmp_path, elements):
    ...
    async def extract(schema, *, fallback, **kwargs):
        ...
        return TaskDecision(operation='answer', answer_status='partial',
            answer='已保留门店地址和套餐98元。菜单详情需在App查看，套餐规则尚不完整。',
            citations=[Citation(artifact_id=context['sources'][0]['artifact_id'], quote=text)])
    ...
    with TestClient(app, headers={"Authorization": "Bearer " + TOKEN}) as client:
        ...
        assert outcome['data']['scope'] == 'task_answer'
        assert result['state']['browser_artifacts'][0]['data']['text'] == text
        ...
        assert calls == ["TaskDecision", "TaskDecision"]
