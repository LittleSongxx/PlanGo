...
最新评测研究：用户将质量展示指标收敛为**任务成功率TSR与事实支持率Groundedness**，Token仅记录。已形成[评测协议](质量评测协议.md)及`eval/quality-v1/`的12条公开dev任务/gold草案、原始来源hash、30条未见留出采集计划和空标注表；金标未人审、留出尚未采集、runner/时钟适配未就绪。`python3 scripts/check_quality_design.py`仅作静态文件校验，不跑模型或生成分数。**本轮研究不解除质量测评暂停，不把旧回归数字或这12条草案当新成绩。**
