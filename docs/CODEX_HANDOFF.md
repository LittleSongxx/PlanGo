...
更新：**2026-09-09，评测驱动的需求编辑与资料分析修复**。工作区：`/home/song/code/Agent/multi-agent/PlanGo`。
...
用户主投方向已确认 **AI应用／Agent开发**；[秋招与作品集建议](秋招与作品集推进建议.md)是辅助背景，不能据此把本次产品实施改成写简历或发布作品集。20个旧评估任务、3–5个独立试用者等仍是建议数量，尚未执行。用户随后明确要求实施两指标评测，已授权并完成下述12例公开dev采集；**大规模质量/角色收益/DOM-Vision sweep仍暂停**。
...
最新评测实施：用户将质量展示指标收敛为**任务成功率TSR与事实支持率Groundedness**，Token仅记录，随后要求“现在开始实施”。[评测协议](质量评测协议.md)与[运行说明/证据](../eval/quality-v1/README.md)现有隔离runner、原始来源包、业务时钟、真实React可见输出导出、独立SQL检查、受控Electron恢复故障、AI辅助标注及确定性聚合。12例known-dev全部采集，actor25调用/34039tokens；judge单例1调用/4461tokens，因无效/缺失证据引用保留pending。23测试＋15subtests及Ruff/导出器自检通过。6项机器状态条件5通过1失败，其余54项待审，**完整标注0/12，TSR/Groundedness为空；金标未人审、30条留出未采集**。旧静态检查的0次运行只描述它本身，不能覆盖本次真实调用。

本批来源manifest=`172bd30fd11bb80c5807a4a5e5d6cfe00e9d6a2a71523f32f0dfd5da270b2697`，原记录`output/quality-v1/dev-20260909T112246Z-eeb85930/`，judge`output/quality-v1/review-judge-20260909T1129/`。8例为原始观测注入，4例为显式受控未保存草案导入，其中2例经过实际Electron；不能当完整商家网页E2E。DEV-05/10改3人后仍2人；DEV-06字段已更新但额外起点澄清；DEV-04/08分析请求进入定位澄清。保存/同请求恢复的局部证据不等于整题成功。原失败不重跑覆盖，后续先复核这些问题与来源/gold，再冻结新批次；较大judge包超当前单轮预算时保留人工待审，不截材料凑分数。所有临时API/窗口/代理已关，独立DB/profile保留，主服务未修改/重启。

首轮采集器提交`c15217d`；随后仅补`Evidence.expired`内部业务时钟及评审输入的初态旧回复排除、固定世界来源/hash/取得时间校验，没有新增actor调用。最终准备包`output/quality-v1/review-corrected-20260909T1139/`组包12例、judge调用0、完整标注0；旧judge失败和代码保留。下次需重新预检当前源码hash，不复用首轮hash运行修改后代码。

**最新产品修复已实施并更新主API/worker**：[评测驱动修复](../eval/plango-quality-fixes/README.md)，代码`7e63cc8`/`aeb798a`/`0eba55c`。DEV-05/10普通“人数”修改最终为3，角色人数和否定保留；DEV-06不再把日期当搜索地区，原起点从未丢失。明确资料分析走既有浏览器读取，模型提取引文，确定性核对单位/金额/人数/日期时段并核算；成人资格仍是条件，缺票价/规则保持未知。首轮5例中分析两题仍失败，补短引文验收后仅重验这2例并完成有界答复，全部旧失败保留。实际25调用/30170tokens，未改题/gold/fixture、未评分或提高预算。完整检查426pytest＋58subtests通过，后续23项定向与Ruff/mypy26文件通过。主服务健康、未决0，原1413业务行及3个登录/身份/回执文件保持；私有备份`output/quality-fixes-main-20260909/`。临时API/窗口已关闭，数据/profile保留。后续优先人审金标/来源/成人资格等歧义，再封存未见集；不能把本次已知失败案例修复当正式质量百分比。
...
| 最新实现 | D1/D2=`34eb7d0`；D3–D6及聊天/黄色主题=`e8e156b`。优惠适用性/同任务选用、单站输入收窄、距离/公交/费用已实现；此前`a05f037`、`87f18e9`、`d6086c1`均保留 |
