# PlanGo 历史实施交接（原 YOYU）

> **历史基线，非当前执行清单。** R0→P3 本轮核心实施已经完成，当前路径为 `/home/song/code/Agent/multi-agent/PlanGo`。新对话先读 [CODEX_HANDOFF.md](CODEX_HANDOFF.md)，复制入口见 [接手提示词](CODEX_接手提示词.md)。下文旧路径、旧缺陷和阶段动作保留为历史定位，不重复迁移或重建数据；最终状态与未覆盖项以新 handoff 和实施进度末尾为准。
...
**当时要求：全面更名为 PlanGo，并按 R0→P3 实施。** 现已完成本轮核心实现与声明范围的验证，详细记录见 [实施进度](实施进度.md)。下文旧路径表格是迁移前定位与兼容依据。展示名用 `PlanGo`，技术标识用 `plango` / `PLANGO_`。旧文件名、路径和测试证据在下文作为迁移前定位依据保留，不能据此继续要求新项目使用旧名称。
...
本段以下为迁移前基线，包含当时未修复的问题；R0→P3 的最终实现、验收与尚未覆盖能力见 [当前交接](CODEX_HANDOFF.md) 和 [实施进度](实施进度.md)末尾。不要重复更名、恢复旧 renderer 执行器，或把下文阶段动作当作新会话直接执行清单。
