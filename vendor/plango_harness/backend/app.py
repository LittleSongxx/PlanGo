...
logger = logging.getLogger("plango")
...
def _event_stream(
    runtime: PlanGoRuntime, run_id: str, request: Request, after: int
) -> StreamingResponse:
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    runtime = PlanGoRuntime(settings)
    ...
    app = FastAPI(
        title="PlanGo — Multi-Agent Local-Life Planner",
    ...
    async def internal_error(_request: Request, exc: Exception):
        logger.exception("unhandled PlanGo request error", exc_info=exc)
        return JSONResponse(
    ...
    async def health_live():
        return {"status": "ok", "service": "plango"}
    ...
    async def index():
        ...
        return {"service": "plango", "docs": "/docs"}
