...
class Settings(BaseSettings):
    """Runtime configuration for the Planora application.
    ...
    app_name: str = "Planora"
    app_version: str = "0.1.0"
    host: str = Field("127.0.0.1", validation_alias="PLANORA_HOST")
    port: int = Field(8000, validation_alias="PLANORA_PORT")
    runtime_profile: Literal["service", "sandbox"] = Field(
        "service", validation_alias="PLANORA_RUNTIME_PROFILE"
    )
    world_provider: Literal["amap", "sandbox"] = Field(
        "amap", validation_alias="PLANORA_WORLD_PROVIDER"
    )
    seed: int = Field(20260903, validation_alias="PLANORA_SEED")
    ...
    sandbox_use_model: bool = Field(False, validation_alias="PLANORA_SANDBOX_USE_MODEL")
    ...
    embedding_api_key: str = Field("", validation_alias="EMBEDDING_API_KEY", repr=False)
    ...
    database_url: str = Field(
        "postgresql+asyncpg://planora:planora@127.0.0.1:5432/planora",
        validation_alias="DATABASE_URL",
    )
    redis_url: str = Field("redis://127.0.0.1:6379/4", validation_alias="REDIS_URL")
    data_dir: Path = Field(Path("./data"), validation_alias="PLANORA_DATA_DIR")
    checkpoint_path: Path = Field(
        Path("./data/planora-checkpoints.sqlite"),
        validation_alias="PLANORA_CHECKPOINT_PATH",
    )
    ...
    max_turns: int = Field(12, validation_alias="PLANORA_MAX_TURNS", ge=1, le=100)
    ...
    max_tool_calls: int = Field(48, validation_alias="PLANORA_MAX_TOOL_CALLS", ge=1, le=1000)
    max_queue_retries: int = Field(3, validation_alias="PLANORA_MAX_QUEUE_RETRIES", ge=1, le=20)
    max_repair_rounds: int = Field(2, validation_alias="PLANORA_MAX_REPAIR_ROUNDS", ge=0, le=20)
    max_context_tokens: int = Field(6000, validation_alias="PLANORA_MAX_CONTEXT_TOKENS")
    max_model_tokens: int = Field(
        12000, validation_alias="PLANORA_MAX_MODEL_TOKENS", ge=256, le=200000
    )
    event_stream: str = Field("planora:runs", validation_alias="PLANORA_EVENT_STREAM")
    memory_stream: str = Field("planora:memory-embed", validation_alias="PLANORA_MEMORY_STREAM")
    worker_group: str = Field("planora-workers", validation_alias="PLANORA_WORKER_GROUP")
    embedded_worker: bool = Field(False, validation_alias="PLANORA_EMBEDDED_WORKER")
    allow_sqlite_fallback: bool = Field(False, validation_alias="PLANORA_ALLOW_SQLITE_FALLBACK")
    allow_redis_fallback: bool = Field(False, validation_alias="PLANORA_ALLOW_REDIS_FALLBACK")
    max_run_seconds: int = Field(300, validation_alias="PLANORA_MAX_RUN_SECONDS", ge=30, le=3600)
    ...
    def canonicalize_legacy_config(cls, values):
        ...
        profile_key = "PLANORA_RUNTIME_PROFILE" if "PLANORA_RUNTIME_PROFILE" in values else "runtime_profile"
        provider_key = "PLANORA_WORLD_PROVIDER" if "PLANORA_WORLD_PROVIDER" in values else "world_provider"
        ...
        if provider not in {"amap", "sandbox"}:
            ...
            set_value("runtime_profile", "PLANORA_RUNTIME_PROFILE", "sandbox")
            profile = "sandbox"
        if profile == "sandbox":
            if "DATABASE_URL" not in values and "database_url" not in values:
                set_value("database_url", "DATABASE_URL", "sqlite+aiosqlite:///./data/planora-sandbox.sqlite")
            if "REDIS_URL" not in values and "redis_url" not in values:
                set_value("redis_url", "REDIS_URL", "redis://127.0.0.1:6399/4")
            if "PLANORA_EMBEDDED_WORKER" not in values and "embedded_worker" not in values:
                set_value("embedded_worker", "PLANORA_EMBEDDED_WORKER", True)
            if "PLANORA_ALLOW_REDIS_FALLBACK" not in values and "allow_redis_fallback" not in values:
                set_value("allow_redis_fallback", "PLANORA_ALLOW_REDIS_FALLBACK", True)
        else:
            # Service always has one external worker and strict dependencies;
            # ignore stale pre-convergence local env toggles.
            set_value("embedded_worker", "PLANORA_EMBEDDED_WORKER", False)
            set_value("allow_sqlite_fallback", "PLANORA_ALLOW_SQLITE_FALLBACK", False)
            set_value("allow_redis_fallback", "PLANORA_ALLOW_REDIS_FALLBACK", False)
        return values
