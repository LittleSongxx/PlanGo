...
from planora.agent.graph import (
    GraphDeps,
...
from planora.agent.state import initial_state
from planora.domain.planning import PlanEngine
from planora.memory.embedding import EmbeddingService
from planora.memory.repository import MemoryRepository
from planora.observability import agent_span
from planora.persistence.actions import ActionLedger
from planora.persistence.database import Database, utc_now
from planora.persistence.runs import RunRepository
from planora.providers import SandboxActionProvider, WorldService
from planora.queue import QueueItem, RunQueue
from planora.settings import Settings
from planora.tools.registry import ToolRegistry
...
class PlanoraRuntime:
    ...
    async def start(self) -> None:
        ...
        if self.embedded_worker:
            self._local_worker_task = asyncio.create_task(
                self._local_worker(), name="planora-local-worker"
            )
        ...
        while True:
            ...
            try:
                ...
                task = asyncio.create_task(
                    self._process_queue_item(item), name=f"planora-run-{item.run_id[:8]}"
                )
    ...
    async def _run_graph(self, run_id: str, resume: dict[str, Any] | None = None, command_id: str | None = None) -> dict[str, Any]:
        ...
        heartbeat = asyncio.create_task(
            self._lease_heartbeat(run_id, lease_owner), name=f"planora-lease-{run_id[:8]}"
        )
        deadline_at = 0.0
        try:
            row = await self.runs.get(run_id)
    ...
    def _jsonable(value: Any) -> Any:
        ...
        if isinstance(value, (list, tuple)):
            return [PlanoraRuntime._jsonable(item) for item in value]
        if isinstance(value, dict):
            return {str(key): PlanoraRuntime._jsonable(item) for key, item in value.items()}
        if hasattr(value, "model_dump"):
            return PlanoraRuntime._jsonable(value.model_dump(mode="json"))
    ...
    def snapshot(row: dict[str, Any] | None) -> dict[str, Any]:
        ...
        return {
            ...
            "state": PlanoraRuntime._jsonable(state),
            "cancel_requested": bool(row.get("cancel_requested")),
            "command_pending": bool(row.get("pending_command")),
            "updated_at": PlanoraRuntime._jsonable(row.get("updated_at")),
        }

    def _ensure_started(self) -> None:
        if not self._started or self.graph is None:
            raise RuntimeError("PlanoraRuntime is not started")
