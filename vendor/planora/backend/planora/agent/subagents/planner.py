...
from planora.agent.contracts import PlaceCandidate, PlanDraft, PlanDraftStop, TripSpec
from planora.agent.model_adapter import ModelAdapter
from planora.domain.planning import place_fits
...
class PlannerAgent:
    ...
    async def run(
        ...
        return await self.model.structured(
            PlanDraft,
            system=(
                "你是 Planora 的 Planner Agent。根据 TripSpec、已观测候选地点和 Advocate 报告，"
