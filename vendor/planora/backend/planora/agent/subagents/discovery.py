...
from planora.agent.contracts import Evidence, Location, PlaceCandidate, TripSpec
from planora.agent.decisions import DiscoveryOutput
        ...
        async def model_queries() -> list[str]:
            ...
            calls = (
                await self.model.tool_calls(
                    system=(
                        "你是 Planora 的 Discovery Agent。只能调用只读地点搜索工具，"
            ...
            decision = DiscoveryOutput(queries=requested[:6]) if requested else await self.model.structured(
                DiscoveryOutput,
                system=(
                    "你是 Planora 的 Discovery Agent。根据结构化出行目标决定需要查询的地点类别。"
                    "只返回短查询词，不编造地点或事实。"
                ),
                user=f"TripSpec：{spec.model_dump_json()}",
                fallback=fallback,
