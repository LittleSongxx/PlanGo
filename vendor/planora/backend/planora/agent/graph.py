...
from planora.agent.decisions import SupervisorDecision
from planora.agent.model_adapter import ModelAdapter
from planora.agent.state import PlanoraState
from planora.agent.subagents import (
    AdvocateAgent,
    CriticAgent,
    DiscoveryAgent,
    PlannerAgent,
...
from planora.agent.subgraphs import (
    advocate_subgraph,
    critic_subgraph,
    discovery_subgraph,
    reflection_subgraph,
...
from planora.domain.planning import PlanEngine, ToolBudgetExceeded, compile_plan_draft, goal_errors
from planora.memory.repository import MemoryRepository
from planora.persistence.actions import ActionLedger
from planora.persistence.runs import RunRepository
from planora.providers.actions import ActionProvider
from planora.providers.world import WorldProvider
from planora.tools.registry import ToolContext, ToolRegistry
...
class GraphDeps:
    ...
    def tool_context(self, state: PlanoraState, *, approved: bool = False) -> ToolContext:
...
def _trace(state: PlanoraState, event: str, **payload: Any) -> list[dict[str, Any]]:
...
def _current_advocate_reports(state: PlanoraState) -> list[Any]:
...
def _can_repair_price_gap(state: PlanoraState, max_rounds: int) -> bool:
...
def checkpoint_serializer() -> JsonPlusSerializer:
    ...
    return JsonPlusSerializer(
        allowed_msgpack_modules=[("planora.agent.contracts", name) for name in names]
    )


def _fallback_decision(state: PlanoraState, deps: GraphDeps) -> SupervisorDecision:
...
def build_graph(deps: GraphDeps, checkpointer: Any | None = None, *, extension=None):
    ...
    async def load_memory(state: PlanoraState) -> dict[str, Any]:
    ...
    async def requirements(state: PlanoraState) -> dict[str, Any]:
    ...
    async def discovery(state: PlanoraState) -> dict[str, Any]:
        spec = state.get("trip_spec")
    ...
    def advocate_fanout(state: PlanoraState) -> list[Send]:
        spec = state.get("trip_spec")
        assert spec is not None
    ...
    async def advocate_worker(state: PlanoraState) -> dict[str, Any]:
    ...
    async def synthesis(state: PlanoraState) -> dict[str, Any]:
        places = _place_candidates(state.get("place_candidates"))
    ...
    async def verify(state: PlanoraState) -> dict[str, Any]:
    ...
    async def critic(state: PlanoraState) -> dict[str, Any]:
        selected = state.get("selected_plan")
        verifier = state.get("verifier")
    ...
    async def propose_actions(state: PlanoraState) -> dict[str, Any]:
        selected = state.get("selected_plan")
        verifier = state.get("verifier")
        ...
        from planora.agent.contracts import ActionProposal
    ...
    async def approval(state: PlanoraState) -> dict[str, Any]:
    ...
    async def replan(state: PlanoraState) -> dict[str, Any]:
    ...
    async def execute(state: PlanoraState) -> dict[str, Any]:
        proposal = state.get("action_proposal")
    ...
    async def reflect(state: PlanoraState) -> dict[str, Any]:
    ...
    async def prepare_ask_user(state: PlanoraState) -> dict[str, Any]:
        verifier = state.get("verifier")
    ...
    async def ask_user(state: PlanoraState) -> dict[str, Any]:
    ...
    async def finalize(state: PlanoraState) -> dict[str, Any]:
    ...
    async def supervisor(state: PlanoraState) -> dict[str, Any]:
    ...
    def after_supervisor(state: PlanoraState) -> str:
        return {
    ...
    def after_approval(state: PlanoraState) -> str:
    ...
    async def advocate_entry(state: PlanoraState) -> dict[str, Any]:
