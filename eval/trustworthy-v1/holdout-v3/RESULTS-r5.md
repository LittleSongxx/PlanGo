...
| conflict/unknown 只回不确定词（<12 实义字符） | 26 | 18 | 22 | **1** |
...
> 「只回不确定词」按 `delivery_substance < 12` 统计（v3 的 `substance_min` 阈值），
> 只数 conflict/unknown 两层；报告里的 `delivery_bare_uncertainty` 是另一个口径
> （非空且无事实断言），v1.4 之后也覆盖「有说明的缺口答复」，不要混用。
