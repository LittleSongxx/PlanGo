...
接续实施时先读 [docs/CODEX_HANDOFF.md](docs/CODEX_HANDOFF.md)，优先看第0节目标、第4节质量基线及第5–7节系统优化与复测入口及第9节最新6+6结果。30例基线及本轮6旧+6新独立AI审核已完成，回归TSR仍0/6、新集仅保存恢复1/6，当前继续从共同机制修复并有界复测，避免逐badcase堆规则；旧集复跑只称回归。R0→P3、结构化编辑、试用、真实门店草案及UI已有证据，真实商家完整准备/履约和独立使用验收仍未完成；[旧实施交接](docs/CODEX_实施交接.md)仅保留历史，不重复迁移/登录，也不把受控测试当真实商家履约。
