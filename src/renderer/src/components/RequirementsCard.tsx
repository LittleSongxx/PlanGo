...
const fieldLabels: Record<keyof RequirementFields, string> = {
  location_name: '出发地点', search_location_name: '搜索中心', max_distance_km: '搜索范围 / 距离上限（公里）',
...
function valuesOf(run: HarnessSnapshot): Record<keyof RequirementFields, string> {
  const spec = row(run.state.trip_spec || run.state.previous_spec)
  return Object.fromEntries(Object.keys(fieldLabels).map(key => [key,
    String(key === 'location_name' ? row(spec.location).name || '' : key === 'search_location_name' ? row(spec.search_location || spec.location).name || '' : spec[key] ?? '')
  ])) as Record<keyof RequirementFields, string>
}

export function RequirementsCard(): JSX.Element | null {
  const run = useStore(s => s.run)
  if (!run?.version || !Object.keys(row(run.state.trip_spec || run.state.previous_spec)).length) return null
...
function RequirementForm({ run }: { run: HarnessSnapshot }): JSX.Element {
  ...
  const disabled = busy || !ready || unresolved || !!run.command_pending || !!run.cancel_requested
  const spec = row(run.state.trip_spec || run.state.previous_spec)
  ...
  return <section aria-label="行程需求" className="max-w-4xl mx-auto plango-card p-5 mb-5">
    <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold text-base text-brand-ink">行程需求</h2><p className="text-xs text-[var(--muted)] mt-1">修改后在当前任务中重新规划，原方案和确认记录保留。</p></div><span className="text-[11px] text-[var(--muted)]">{spec.timezone === 'Asia/Shanghai' ? '北京时间' : String(spec.timezone || '')}</span></div>
    <form onSubmit={event => {
      event.preventDefault()
      if (disabled || !changed.length) return
      const numeric = new Set(['party_size', 'budget', 'per_person_budget', 'max_distance_km'])
      ...
      void edit({ expected_version: run.version!, fields })
    }}>
      <fieldset disabled={disabled} className="grid grid-cols-2 gap-3 mt-4 disabled:opacity-60">
        {(Object.keys(fieldLabels) as (keyof RequirementFields)[]).map(key => <label key={key} className={`block text-xs text-neutral-600 ${['location_name', 'search_location_name'].includes(key) ? 'col-span-2 sm:col-span-1' : ''}`}>
          {fieldLabels[key]}
          {key === 'travel_mode' ? <select aria-label={fieldLabels[key]} className="plango-field mt-1" value={values[key]} onChange={event => setValue(key, event.target.value)}><option value="driving">驾车</option><option value="walking">步行</option><option value="transit">公共交通</option></select>
            : <input aria-label={fieldLabels[key]} className="plango-field mt-1" value={values[key]} onChange={event => setValue(key, event.target.value)}
              type={key === 'visit_date' ? 'date' : key === 'time_window_start' ? 'time' : ['party_size', 'budget', 'per_person_budget', 'max_distance_km'].includes(key) ? 'number' : 'text'}
              required={changed.includes(key) && ['location_name', 'search_location_name', 'party_size'].includes(key)} maxLength={200}
              min={key === 'party_size' ? 1 : key === 'max_distance_km' ? 0.1 : 0} max={key === 'party_size' ? 12 : key === 'max_distance_km' ? 50 : 1_000_000}
              step={key === 'party_size' ? 1 : key === 'max_distance_km' ? 0.1 : key === 'time_window_start' ? 60 : 'any'}
              placeholder={['budget', 'per_person_budget'].includes(key) ? '不限' : key === 'max_distance_km' ? '默认附近 5 公里，无距离硬上限' : '待确认'} />}
        </label>)}
      </fieldset>
      <p className="text-[11px] text-[var(--muted)] mt-3 leading-5">距离上限同时用于搜索和路线核验。预算留空即取消对应上限；日期或时间留空表示待确认。</p>
      <div className="flex flex-wrap items-center gap-2 mt-3">
        <button type="submit" disabled={disabled || !changed.length} className="plango-primary disabled:opacity-40">保存需求并重新规划</button>
