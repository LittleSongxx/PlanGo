...
export function HistoryDrawer(): JSX.Element | null {
  ...
  return <DialogShell label="历史会话" onClose={() => setOpen(false)} className="plango-dialog-history w-[400px]">
    ...
    <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-3">
      {!sessions.length ? <div className="flex flex-col items-center justify-center py-20 text-center"><span className="flex w-16 h-16 items-center justify-center rounded-2xl bg-brand-soft text-[#6c927b]"><MessageSquare size={25} strokeWidth={1.5} /></span><h3 className="mt-5 font-semibold text-sm">还没有保存的会话</h3><p className="mt-2 text-xs text-[var(--muted)] leading-6 max-w-[230px]">开始一次对话后，可以在这里找回任务与已有结果。</p></div>
        : sessions.map(session => <article key={session.id} className={`relative group rounded-2xl border transition-colors ${session.id === activeId ? 'bg-brand-soft border-[#bcd9c8]' : 'bg-white border-[var(--line)] hover:border-[#9ec2ac]'}`}>
          <button data-history-entry aria-label={`打开会话：${session.title}`} onClick={() => restore(session.id)} className="block w-full text-left p-4 pr-11">
            <div className="flex items-center gap-2"><MessageSquare size={14} className="text-[#719480] shrink-0" /><span className="text-[13px] font-medium truncate">{session.title}</span></div>
