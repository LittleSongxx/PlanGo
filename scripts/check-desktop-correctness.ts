...
import { allowsGeolocation } from '../src/main/permissions'
...
check('unknown event remains neutral', () => assert.equal(event('new_event'), 'idle'))
...
check('requirements phase alone cannot declare an event successful', () => assert.equal(event('new_event', {}, 'REQUIREMENTS_READY'), 'idle'))
