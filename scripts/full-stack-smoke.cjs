// Actual PlanGo Python backend + Electron + local real DOM. --deployed uses the configured Docker/model service.
...
function deploymentConfig() {
  ...
  assert.equal(result.status, 0, 'Cannot parse deployment configuration; values were not printed')
...
process.env.PLANGO_BACKEND_TOKEN = token
...
async function startBackend(port) {
  ...
  backend = spawn(python, ['-m', 'uvicorn', 'plango.app:create_app', '--factory', '--host', '127.0.0.1', '--port', String(port), '--no-access-log'], {
    cwd: work,
    env: { PATH: process.env.PATH, LANG: 'C.UTF-8', PYTHONPATH: [join(root, 'backend'), join(root, 'vendor/plango_harness/backend')].join(delimiter), PYTHONDONTWRITEBYTECODE: '1',
      PLANGO_BACKEND_TOKEN: token, PLANGO_DATA_DIR: process.env.PLANGO_DATA_DIR, PLANGO_RUNTIME_PROFILE: 'desktop', PLANGO_REDIS_URL: 'local://',
...
async function main() {
  ...
  for (const current of [session.defaultSession, session.fromPartition('persist:plango')]) current.webRequest.onBeforeRequest((details, callback) => callback({ cancel: /^https?:/.test(details.url) && !details.url.startsWith(fixtureUrl) && !details.url.startsWith(backendUrl + '/') }))
